...
class Text2VectorDVM(EZDVM):
    ...
    def __init__(self):
        ...
        super().__init__(kinds=self.kinds, nostr_sdk_log_level=None, log_full_events=True)
    ...
    async def do_work(self, event: Event):
        ...
        try:
            ...
            if len(texts) > MAX_BATCH:
                self.logger.warning(f"Request exceeds max batch size: {len(texts)} > {MAX_BATCH}")
            ...
            end_time = time.time()
            self.logger.info(f"Embeddings generated in {end_time - start_time:.2f} seconds")
            ...
            self.logger.info(f"Response event created with ID: {resp_event.id().to_hex()}")
